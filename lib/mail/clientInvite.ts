...
<body style="margin:0;padding:0;background:${colors.paper};">
  <div style="display:none;max-height:0;overflow:hidden;opacity:0;mso-hide:all;">
    Finish registration for ${org} on the ${name} client portal — link expires in 72 hours.
  </div>
                ...
                <tr>
                  <td style="padding:28px 28px 8px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;font-size:16px;line-height:1.55;color:${colors.ink};">
                    <p style="margin:0 0 16px;color:${colors.muted};">
                      ...
                      <strong style="color:${colors.ink};">${org}</strong>
                      on the ${name} client portal.
                    </p>
                    <p style="margin:0 0 22px;color:${colors.muted};">
                      Complete registration to set your password and confirm organisation details. You&rsquo;ll add billing details on your profile before the dashboard unlocks.
                    </p>
                  </td>
                </tr>
                <tr>
                    ...
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${colors.soft};">
                      <tr>
                        <td style="padding:16px 18px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;font-size:13px;line-height:1.5;color:${colors.ink};">
                          ...
                          This invite expires in <strong>72 hours</strong>. Accounts are invite-only — if you weren&rsquo;t expecting this email, you can ignore it.
                        </td>
                      </tr>
                    </table>
                  </td>
...
function buildInviteText(input: {
  ...
  return [
    `You're invited to the ${site.name} client portal`,
    "",
    `You've been invited to join ${input.organisationName} on the ${site.name} client portal.`,
    "",
    "Complete registration to set your password and confirm organisation details. You'll add billing details on your profile before the dashboard unlocks:",
    input.inviteUrl,
    "",
...
export async function sendClientInviteEmail(input: {
  ...
  const subject = `You're invited — ${input.organisationName} client portal`;
